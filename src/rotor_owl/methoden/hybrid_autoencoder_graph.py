"""
Hybrid-Methode: Autoencoder + Vektorbasiert.

Kombiniert parametrische (Autoencoder - Pattern-basiert) und attributive (Vektorbasiert - Distanz-basiert) Ähnlichkeit
für optimale Rotor-Vergleiche.

WARUM Vektorbasiert statt Graph-Embeddings?
...
- Vektorbasiert zeigt 54.7% Range (exzellente Diskriminierung)
- Autoencoder (93.1% Range) und Vektorbasiert (54.7% Range) sind komplementär:
  * Autoencoder: Findet Pattern-basierte Ähnlichkeiten (gleiche Feature-Korrelationen)
  * Vektorbasiert: Findet Attribut-basierte Ähnlichkeiten (gleiche Absolutwerte)

HINWEIS: Diese Datei heißt "hybrid_autoencoder_graph.py" aus historischen Gründen,
verwendet aber Vektorbasiert statt Graph-Embeddings (siehe Begründung oben).
"""
...
def berechne_topk_aehnlichkeiten_hybrid_ae_graph(
    ...
    gewicht_graph: float = 0.5,  # Jetzt Vektorbasiert-Gewicht
    latent_dim: int = 8,
    ...
    """
    Hybrid-Methode: Kombiniert Autoencoder (Pattern-basiert) + Vektorbasiert (Attribut-basiert).
    ...
    Args:
        ...
        gewicht_graph (float): Gewicht für Vektorbasiert-Ähnlichkeit
    ...
    # Vektorbasierte Ähnlichkeit (Attribut-basiert)
    vektor_results = berechne_topk_aehnlichkeiten_vektorbasiert(
        query_rotor_id=query_rotor_id,
        rotor_ids=rotor_ids,
    ...
    vektor_scores = {rotor_id: (sim, sim_kat) for rotor_id, sim, sim_kat in vektor_results}
    ...
    for rotor_id in rotor_ids:
        ...
        vektor_sim, vektor_sim_kat = vektor_scores.get(rotor_id, (0.0, {}))

        # Gewichtete Gesamt-Similarity
        hybrid_sim = gewicht_autoencoder * ae_sim + gewicht_graph * vektor_sim

        # Kombinierte Kategorie-Similarities
        combined_sim_kat = {}
        for kategorie in gewichtung_pro_kategorie.keys():
            ae_kat_sim = ae_sim_kat.get(kategorie, 0.0)
            vektor_kat_sim = vektor_sim_kat.get(kategorie, 0.0)
            combined_sim_kat[kategorie] = (
                gewicht_autoencoder * ae_kat_sim + gewicht_graph * vektor_kat_sim
            )

        # Similarity pro Methode
        sim_pro_methode = {
            "Autoencoder": ae_sim,
            "Vektorbasiert": vektor_sim,
        }
