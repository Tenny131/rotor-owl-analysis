...
if __name__ == "__main__":
    ...
    print("\n[MAIN] --- Loading real parameter data from JSON (WVSC)...")
    features_by_rotor = fetch_all_features_from_json()
    print(f"  {len(features_by_rotor)} Rotoren aus JSON geladen.")

    # Konvertiere json_parser-Format in DataFrame für data_mapper
    # json_parser: features_by_rotor[rotor_id]["params"][(component, param)] = {"value", "unit", "ptype"}
    # data_mapper erwartet: Design_ID, Component_ID, Parameter_ID, ParamType_ID, Value, Unit
    zeilen = []
    for rotor_id, rotor_daten in features_by_rotor.items():
        for (component, param), werte in rotor_daten["params"].items():
            if werte["value"] is None:
                continue
            zeilen.append(
                {
                    "Design_ID": rotor_id,
                    ...
                    "Parameter_ID": param,
                    "ParamType_ID": werte["ptype"],
                    "Value": werte["value"],
                    "Unit": werte["unit"],
                }
            )
    df_parameters_filled = pd.DataFrame(zeilen)
    df_parameters_filled["Name"] = df_parameters_filled["Parameter_ID"]
    df_parameters_filled["Definition"] = ""
    print(f"  {len(df_parameters_filled)} Parameterwerte konvertiert.")
    # ======================print sheet names =========================
    ...
    # Ergänze Komponenten-Klassen, die in den Realdaten vorkommen aber
    # nicht im Excel-Sheet definiert sind
    # Mapping: Component_ID → Klassen-Label
    _comp_labels = {
        "C_WELLE": "Welle",
        "C_WELLEENDE": "Welleende",
        "C_AKTIVTEIL": "Aktivteil",
    ...
    realdaten_komponenten = df_parameters_filled["Component_ID"].unique()
    for comp_id in realdaten_komponenten:
        label = _comp_labels.get(comp_id) or str(comp_id).replace("C_", "").title()
        comp_uri = IMS[label]
        g.add((comp_uri, RDFS.subClassOf, IMS["Artefact"]))
        g.add((comp_uri, RDFS.label, Literal(label)))
    print(
        f"\n[MAIN] --- Component classes mapped successfully ({len(realdaten_komponenten)} Komponenten)."
    )

    # ============ parameter sheet =========================
    # Für Realdaten: Parameter-Klassen aus den tatsächlich vorhandenen
    # json_parser-Parametern ableiten (nicht aus dem alten Excel-Sheet,
    # das andere Parameter definiert als die JSON-Realdaten enthalten).
    print("\n[MAIN] --- Mapping parameter classes from real data...")
    parent = IMS["Object"]
    child = IMS["Parameter"]
    g.add((child, RDFS.subClassOf, parent))
    g.add((child, RDFS.label, Literal("Parameter")))

    # Eindeutige Parameter-IDs aus dem konvertierten DataFrame
    unique_params = df_parameters_filled["Parameter_ID"].unique()
    for param_id in unique_params:
        fragment = param_id.strip().replace(" ", "_")
        param_class = IMS[fragment]
        g.add((param_class, RDFS.subClassOf, IMS["Parameter"]))
        g.add((param_class, RDFS.label, Literal(param_id)))
    print(f"\n[MAIN] --- {len(unique_params)} Parameter-Klassen aus Realdaten erstellt.")
    ...
    output_file = os.path.join(output_dir, "rotor_ontologie_realdaten.owl")
